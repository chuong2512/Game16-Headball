...
public class UnityRateAll
{
	...
	public void SendToRating() 
	{
		...
		if (debug)
			Debug.Log("UnityRate.RegisterUserRated for version "+version);
		...
		if (debug) 
		{
			...
			/*if (PreviewLabs.PlayerPrefs.GetBool("GoogleRated",false) == false)
			{
				PreviewLabs.PlayerPrefs.SetBool("GoogleRated",true);
				int gem = 10;
				gem += PreviewLabs.PlayerPrefs.GetInt("GemCount", 0);
				PreviewLabs.PlayerPrefs.SetInt("GemCount", gem);
				gameManeger gm = GameObject.Find("GameManeger").GetComponent<gameManeger>();
				gm.GemCount += 10;
				gm.scoreNumberGem.text = gm.GemCount.ToString();
